...
def get_element_for_wait(driver, by, by_s, timeout=1):
    try:
...
def get_include_hide_element_for_wait(driver, by, by_s, timeout=1):
    try:
...
def get_include_hide_elements_for_wait(driver, by, by_s, timeout=1):
    try:
        ele = WebDriverWait(driver, timeout).until(
...
def get_elements_for_wait(driver, by, by_s, timeout=1):
    try:
        WebDriverWait(driver, timeout).until(ec_obj.presence_of_element_located((by, by_s)))
